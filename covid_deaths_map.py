...
# Group the data by state and year_month and sum the total deaths
df = df.groupby(['state', 'year_month']).agg({'tot_death': 'sum'}).reset_index()

# Create the map using Plotly Express
fig = px.choropleth(df, locations='state', locationmode='USA-states', color='tot_death', color_continuous_scale='Reds', scope='usa', animation_frame='year_month')
