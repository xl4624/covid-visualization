...
# Group the data by year_month and sum the total cases and deaths
df = df.groupby('year_month').agg({'tot_cases': 'sum', 'tot_death': 'sum'}).reset_index()

# Create the line chart using Plotly Express
fig = go.Figure()
fig.add_trace(go.Scatter(x=df['year_month'], y=df['tot_cases'], name='Total Cases'))
fig.add_trace(go.Scatter(x=df['year_month'], y=df['tot_death'], name='Total Deaths', yaxis='y2'))
